...
abstract class TweetSet {
  ...
  def mostRetweetedAcc(candidateTweet: Tweet): Tweet
...
class Empty extends TweetSet {
  ...
  def mostRetweeted: Nothing = throw new NoSuchElementException("Empty set contains no tweets.")

  def mostRetweetedAcc(candidateTweet: Tweet): Tweet = candidateTweet
...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def mostRetweeted: Tweet = {
    mostRetweetedAcc(new Tweet("foo","bar",0))
  }

  def mostRetweetedAcc(candidateTweet: Tweet): Tweet =
    if (elem.retweets >= candidateTweet.retweets)
      right.mostRetweetedAcc(left.mostRetweetedAcc(elem))
    else right.mostRetweetedAcc(left.mostRetweetedAcc(candidateTweet))
